...
void light_init() {
    ...
    uint8_t enable_buf[] = { 0xA0, 0x03 };
    uint8_t status = i2c_io(write_addr, enable_buf, 2, NULL, 0);
...
float get_light() {
    ...
    uint8_t status = i2c_io(write_addr, reg_addr, 1, light_buf, 4);
    if (status != 0) {
        lcd_moveto(2, 0);
        snprintf(buffer, sizeof(buffer), "Read Err: %02X", status);
        lcd_stringout(buffer);
        _delay_ms(2000);    // Give time to see error
        return -1;
    }
    ...
    float atime = 100.0;    // ms (integration time default)
