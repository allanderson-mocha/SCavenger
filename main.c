...
// #define STEP_GOAL 69
// #define ALTITUDE_THRESHOLD 10.0
...
//         static char just_transitioned = 1;
...
//                         game.target_longitude = longitude + 0.0005f;
...
// extern char gps_buffer[GPS_BUFFER_SIZE]; // Defined in gps.c

// void display_gps_sentence(const char* sentence);

// int main(void) {
//     gps_init();  // Initialize UART for GPS
//     lcd_init();  // Initialize LCD display

//     lcd_moveto(0, 0);
//     lcd_stringout("Waiting for GPS...");
//     _delay_ms(2000); // Give GPS time to start

//     while (1) {
//         read_gps_sentence();  // Read one full NMEA sentence into gps_buffer

//         lcd_writecommand(0x01); // Clear the LCD
//         lcd_moveto(0, 0);

//         display_gps_sentence(gps_buffer); // Display the full sentence

//         _delay_ms(1000); // Update every second (optional)
//     }
...
//     for (uint16_t i = 0; sentence[i] != '\0'; i++) {
//         lcd_moveto(row, col);
//         lcd_writedata(sentence[i]);

//         col++;
//         if (col >= 40) { // 20 columns per row
//             col = 0;
//             row++;
//             if (row >= 2) break; // Only 4 rows available
//         }
//     }
// }
...
// extern char gps_buffer[GPS_BUFFER_SIZE]; // Declared in gps.c

// void display_lat_lon(float latitude, float longitude);
...
//         display_lat_lon(latitude, longitude);

//         _delay_ms(1000); // Update once per second
//     }

//     return 0;
// }

// // Helper: Display parsed latitude and longitude on the LCD
// void display_lat_lon(float latitude, float longitude) {
//     char buffer[17];

//     // Latitude first line
//     int32_t lat_whole = (int32_t)latitude;
//     int32_t lat_frac = (int32_t)((latitude - lat_whole) * 1000000L);
//     if (lat_frac < 0) lat_frac = -lat_frac;

//     lcd_moveto(0, 0);
//     lcd_stringout("Lat:");
//     snprintf(buffer, sizeof(buffer), "%ld.%06ld", lat_whole, lat_frac);
//     lcd_stringout(buffer);

//     // Longitude second line
//     int32_t lon_whole = (int32_t)longitude;
//     int32_t lon_frac = (int32_t)((longitude - lon_whole) * 1000000L);
//     if (lon_frac < 0) lon_frac = -lon_frac;

//     lcd_moveto(1, 0);
//     lcd_stringout("Lon:");
//     snprintf(buffer, sizeof(buffer), "%ld.%06ld", lon_whole, lon_frac);
//     lcd_stringout(buffer);
// }
...
int main(void) {
    ...
    while (1) {
        if (gps_sentence_ready()) {
            ...
            display_sentence(gps_buffer);

            gps_reset();  // Very important! Otherwise ISR won't overwrite
            _delay_ms(1000);  // Slow down refresh
        }
    }
}
...
void display_sentence(const char* sentence) {
    ...
    for (uint16_t i = 0; sentence[i] != '\0'; i++) {
        ...
        col++;
        if (col >= 40) {  // 16 chars wide
            col = 0;
            row++;
            if (row >= 2) break; // Only two lines available
        }
    }
}
